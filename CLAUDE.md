...
Status (2026-09-21): Phases 0–4 implemented + bases; game cut down to the incremental basics (main-base level, transport speed, streak cap; no solver-tier automation, drones or consumables for sale); mine blasts disable bases. 36 core tests green, Phase 1 manual play-test still to do.
...
- **Finished numbers disappear.** A number with no Unknown neighbour dims to 25 % opacity while an unsettled flag is still next to it, and fades out once none is; opened tiles fade, and cascades ripple from their start cell (`CellChange.from`).
